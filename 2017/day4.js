...
function solution(passphrases){
    const validPhrases = passphrases.split("\n").filter((passphrase) => {
        const words = passphrase.split(" ");
        const uniqueArray = words.filter((word, position, arr) => arr.indexOf(word) === position);
        return words.length === uniqueArray.length;
    });
    return validPhrases.length;
}

console.log(`First Solution: ${solution(input)}`);

function solution2(passphrases){
    const validPhrases = passphrases.split("\n").filter((passphrase) => {
        const sortedWords = passphrase.split(" ").map((word) => word.split('').sort().join(''));

        const uniqueArray = sortedWords.filter((word, position, arr) => arr.indexOf(word) === position);
        return sortedWords.length === uniqueArray.length;
    });
    return validPhrases.length;
}

console.log(`Second Solution: ${solution2(input)}`);